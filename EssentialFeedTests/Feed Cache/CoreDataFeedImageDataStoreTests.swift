...
class CoreDataFeedImageDataStoreTests: XCTestCase {
    func testRetrieveImageDataDeliversNotFoundWhenEmpty() {
        let sut = makeSUT()
        
        expect(sut, toCompleteRetrievalWith: notFound(), for: anyURL())
    }
    
    func testRetrieveImageDataDeliversNotFoundWhenStoreDataURLDoesNotMatch() {
        let sut = makeSUT()
    ...
    func testRetrieveImageDataDeliversFoundDataWhenThereIsAStoredImageDataMatchingURL() {
        let sut = makeSUT()
    ...
    func testRetrieveImageDataDeliversLastInsertedValue() {
        let sut = makeSUT()
        let firstStoredData = Data("first".utf8)
        let lastStoredData = Data("second".utf8)
        let url = URL(string: "http://a-url.com")!
    ...
    func testSideEffectsRunSerially() {
        let sut = makeSUT()
        let url = anyURL()
