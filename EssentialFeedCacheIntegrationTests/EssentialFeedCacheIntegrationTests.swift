...
//
//  Created by Inna Chystiakova on 30/03/2025.
//
...
final class EssentialFeedCacheIntegrationTests: XCTestCase {
    ...
    func testSaveOverridesItemsSavedOnASeparateInstance() {
    ...
    func testLoadImageDataDeliversSavedDataOnASeparateInstance() {
    ...
    func testSaveImageDataOverridesSavedImageDataOnASeparateInstance() {
    ...
    func testValidateFeedCacheDoesNotDeleteRecentlySavedFeed() {
        let feedLoaderToPerformSave = makeFeedLoader()
    ...
    func testValidateFeedCacheDeletesFeedSavedInADistantPast() {
    ...
    private func save(_ feed: [FeedImage], with loader: LocalFeedLoader, file: StaticString = #file, line: UInt = #line) {
        ...
        loader.save(feed) { result in
            if case let Result.failure(error) = result {
                XCTFail("Expected to save feed seccessfully, got \(error) instead", file: file, line: line)
            }
            saveExp.fulfill()
        }
    ...
    private func save(_ data: Data, for url: URL, with loader: LocalFeedImageDataLoader, file: StaticString = #file, line: UInt = #line) {
        ...
        loader.save(data, for: url) { result in
            if case let Result.failure(error) = result {
                XCTFail("Expected to save image data seccessfully, got \(error) instead", file: file, line: line)
            }
            saveExp.fulfill()
        }
        wait(for: [saveExp], timeout: 1.0)
