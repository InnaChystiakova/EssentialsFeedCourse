...
//
//  Created by Inna Chystiakova on 20/09/2024.
//
...
final class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    // MARK: - Tests
    func testEndToEndServerGETFeedResultMatchesFixedTestAccountData() {
        
        switch getFeedResult() {
        case let .success(imageFeed)?:
            XCTAssertEqual(imageFeed.count, 8, "Expected 8 images in the test account image feed")
            
            //Variat 1 (iteration can be annoying to check the wrong data inside the loop)
            /*
            items.enumerated().forEach { (index, item) in
                XCTAssertEqual(item, expectedItem(at: index), "Unexpected item values at index \(index)")
            }*/
            
            // Variant 2 (more easy to find an element with wrong value)
        ...
        default:
            XCTFail("Expected successful feed result, gor no result instead")
        }
    }

    func testEndToEndTestServerGETFeedImageDataResultMatchesFixedTestAccountData() {
